...
var resourceDescriptions = map[string]string{
	...
	"gateway":                "Every Internet, NAT, and Service Gateway in the compartment (and VCN, if scoped), merged into one list — pick this to see them all at once instead of one type at a time.",
	"igw":                    "An internet gateway lets traffic travel between a VCN's public subnets and the internet. A VCN needs one only if it has a public subnet, and traffic to/from that subnet is controlled by the VCN's route tables and security rules, not the gateway itself.",
	"nat-gateway":            "A NAT gateway gives instances in a private subnet outbound-only access to the internet — they can initiate connections out, but nothing from the internet can initiate a connection in. Unlike an internet gateway, it never gives instances a public IP.",
	"service-gateway":        "A service gateway lets instances in a VCN privately access Oracle services (like Object Storage) over the Oracle network fabric instead of the public internet, without needing internet gateways, NAT gateways, or public IPs.",
	...
	"oke-node-pool":          "A node pool is a subset of an OKE cluster's worker nodes that share the same shape, image, and configuration. A cluster can have several pools with different node shapes for different workloads. Only browsable scoped to one cluster (Enter on an oke row).",
