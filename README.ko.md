...
<details>
<summary><strong>리소스 32종</strong></summary>
...
| --- | --- |
| `j` / `k` (또는 방향키) | 위/아래 이동 |
| `Enter` | Compartment: 상세(YAML) 보기 · VCN: 이 VCN의 Subnet/Route Table/Security List/Gateway만 모은 피커, `i`와 동일 · DRG: 이 DRG의 Attachment/Route Table/Route Distribution만 모은 피커 · OKE: Node Pools/Add-ons 메뉴 · Load Balancer: Listeners/Backend Sets/Routing Policies/Rule Sets/Path Route Sets/Certificates/Hostnames 메뉴 · Instance(`--write` 필요): 액션 메뉴(start/stop, 타이핑 확인 필요) 플로팅 · 그 외: 동작 없음 |
...
| `i` | *(VCN, DRG, OKE, 또는 Load Balancer 행에서)* 그 행에서의 `Enter`와 동일 |
